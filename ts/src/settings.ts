 ...
 *
 * This is not cosmetic (the core repository's C ABI specification §Settings): `chtypes_now_epoch_nanos` is a
 ...
 * (clock control and admission budgets — the core repository's C ABI specification §Settings), by name.
 *
    ...
    if (typeof value === 'string') {
      ...
      throw new ChtypesError(
        `chtypes: setting ${JSON.stringify(key)} must be a string or a bigint, got ` +
          ...
          `(the core repository's C ABI specification §Settings).`,
      );
    }
  }
