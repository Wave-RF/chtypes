...
The default build is **dlopen-only**: it compiles with cgo (for `dlfcn`) but links nothing, includes no header and needs no build tree. That is what a consumer's `go get` produces, and everything in this reference except the section marked *linked build* is available there.
