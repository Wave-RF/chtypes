...
| --------------------------------------------- | ---------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------- |
| `NewRegistry(dir, opts...)`                   | the `manifest.json` scan; dlopen only for a `WithPreload`ed line | artifact dir, or `""` for the search path alone — either way the manifests are read and nothing is dlopen'd → `*Registry`; options `WithAutoFetch`, `WithFetchOptions`, `WithVerifyChecksums`, `WithPreload` | E                                                                                         |
...
| `Registry.Versions()`                         | —                                                                | minor lines, numerically sorted — every line it can answer for, loaded or discovered on the search path; it opens nothing                                                                                    | —                                                                                         |
...
| `LoadedSchema.Row(f, raw, opts...)` / `RowWithSettings(f, raw, settings, opts...)` | `chs_row`                                    | format, raw bytes (+ `WithColumns`, revision 5) → `RowResult`                                                                                                                             | E only; the verdict is in `Outcome`                                                                            |
| `LoadedSchema.Rows(f, body, settings, opts...)`                                    | `chs_rows`                                   | format, body bytes, settings (+ `WithColumns`) → `BatchResult`                                                                                                                            | E only; verdict in `Outcome` / `EngineRows`                                                                    |
| `LoadedSchema.RowsExport(f, body, settings, exportFormat, opts...)`                | `chs_rows` — the same ONE call               | adds `Payload` + `Spans` + `ExportDeclined`, and the document groups `DocValues` / `DocTransforms` / `DocDefaults` (none = lean) or `WithColumns`, in any mix. `ExportNone` = no bytes    | E only; a bad export format or flag bit answers `Unsupported` in the result                                    |
...
| `LoadedSchema.ParseBlock(f, body, settings, opts...)` | `chs_block_parse`                    | parse a body ONCE (+ `WithColumns`, revision 5) → `*Block`, evaluable by K filters with no re-parse                                                             | S (115, framing, decode fault), U, E                                                                  |
...
| `Library.ReconstructDDL(cols)`                                                      | discovered columns → a column-declaration list; the names are spelled by `Library.QuoteIdentifier` |
