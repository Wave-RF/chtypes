...
| `Library.quote_identifier(name)` / `quote_identifier_if_needed(name)` / `quote_literal(text)` | the quoting trio                                       | the spelling ClickHouse itself prints | `UnsupportedError` (artifact predates it); `SchemaError` (a guarded exception's code)                                                                |
