...
| ------------------------------------------------------------------------------------------ | ------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `Registry(dir=None, *, timezone="UTC", verify_hashes=False, autofetch=None, preload=None)` | the manifest scan; `chs_*` only for a `preload`ed line | `Registry` — walks the search path (`.search_path`; `.directory` is where a fetch writes), loads nothing unless `preload` names a line | `RegistryError` (an explicit directory that does not exist or cannot be read; no directory on the search path holding a readable `<minor>/manifest.json` — both suppressed by `autofetch`); `ArtifactMissingError` (a `preload` entry no directory holds) |
| `Registry.versions()`                                                                      | —                                                      | `tuple[str, ...]` of the lines it can answer for — loaded OR discovered on the search path — in release order                          | —                                                                                                                                                                                                                                                         |
| `Registry.libraries()`                                                                     | —                                                      | `tuple[Library, ...]` of the lines it has OPENED, in release order; it opens nothing                                                   | —                                                                                                                                                                                                                                                         |
...
| `Library.quote_identifier(name)` / `quote_identifier_if_needed(name)` / `quote_literal(text)` | the three quoting functions                            | the spelling ClickHouse itself prints | `UnsupportedError` (artifact predates it); `SchemaError` (a guarded exception's code)                                                                |
...
| `Schema.compile_filter(expr, *, params=None)`                           | `chs_filter_compile`                   | `Filter`                                                           | `SchemaError` (47, **456** unbound param, **457** unparseable value); `UnsupportedError` (clock reads, artifact predates the three filter functions) |
...
| `Library.reconstruct_ddl(cols)`                                                 | a column-declaration `str`; names spelled by `quote_identifier` | `ValueError`, `UnsupportedError`                |
