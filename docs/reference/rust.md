...
With an explicit `--dest`, "already installed" means installed *in that directory*: a container build's `--dest /opt/chtypes/artifacts` is never satisfied by the builder's own cache.
