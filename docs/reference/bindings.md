...
**How a binding may implement it.** Two independent detectors run and their union is reported (`go/chtypes/transform.go`):
...
5. **Count the declines.** This is a decline that real traffic can hit, so it belongs in the `unsupported`-rate metric a consumer keeps (the core repository's `docs/production-plan.md` §9), not only in a log line.
