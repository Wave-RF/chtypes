...
⚠️ **Zero in both directions is not the same as agreeing with a server.** Cases remain in which both sides accept a row and **store different values**, and in which both sides refuse it and **report different codes**. Neither is an accept-or-reject disagreement, so the no-budget rule above does not cover them, and this page does not describe them yet. Read the guarantee as exactly what it says — about the verdict, not about the value or the error.
