...
| **Python** | `ctypes` releases the GIL for the whole duration of a foreign call, so the GIL is *not* the exclusion. One writer-preferring readers-writer lock per loaded **image** (row calls, compiles and declarations hold it shared; `set_default_settings` and `close` hold it exclusively) plus one plain lock per `Schema`. |
