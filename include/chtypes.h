 ...
 * are therefore NOT this format; see the C ABI contract.
 *
 ...
 * code 115, exactly as a server rejects the whole query (the C ABI contract,
 * Settings rule 2). */
 ...
 *                  list — see the C ABI contract §Compile-time settings). A partial
 ...
 * future flag can never silently mean nothing. The C ABI contract §Document flags
 ...
 * below). §Filters and §Blocks of the C ABI contract are normative in full;
 * this header states the load-bearing parts.
 *
 ...
 * (the C ABI contract §Filters, Query parameters).
 *
