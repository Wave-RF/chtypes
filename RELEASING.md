...
2. `CHANGELOG` entry naming the ABI revision the release speaks (`4` today) and the ClickHouse lines the golden set was generated on.
3. Run that package's suite against a registry, and `scripts/check-standalone.sh` for Go.
