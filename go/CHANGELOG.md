...
- **The `default_materialised` transform reason is now `default_materialized`**, and the constant naming it is renamed: `ReasonDefaultMaterialised -> ReasonDefaultMaterialized`. ClickHouse's own keyword is `MATERIALIZED` — its parser rejects `MATERIALISED` outright with a syntax error — so the reason naming that concept now matches the system it describes. Code comparing against the old string or constant must be updated.
