...
- **`SourceEphemeralInput` / `SourceMaterializedInput`** name `columns_json`'s two new `Value.Source` provenances (revision 5, issue #53): a listed `EPHEMERAL` column's read value (`"ephemeral_input"` — reported in the column results, never stored, never exported) and a listed `MATERIALIZED` column's supplied value under `insert_allow_materialized_columns=1` (`"materialized_input"` — stored, replacing the expression). Recognition and the parity rows only: `Value.Source` already passed every `src` spelling through verbatim (Go never validated it against a closed set), so both new values already round-trip correctly — this adds the named constants a caller can compare against instead of a bare string literal. The artifact-backed behavior tests (a listed EPHEMERAL/MATERIALIZED column actually producing these values) need a revision-5 artifact, none of which is fetchable yet, and are deferred.
