...
//! `spec/bindings.md` is the prose that explains why. This file is Rust's half
...
fn rust_values() -> BTreeMap<&'static str, Val> {
    ...
    // chs_format — the numbers are frozen (spec/c-abi.md §Types and schemas).
    ...
    m.insert("reason::DEFAULT_MATERIALISED", s(reason::DEFAULT_MATERIALISED));
    ...
    // The canonical discovery queries, carried verbatim (spec/bindings.md §Discovery).
