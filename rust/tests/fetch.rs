...
//! in `spec/fixtures/fetch/` (generated in the core repository, never edited
...
fn fixtures_dir() -> Option<PathBuf> {
    let dir = std::env::var_os("CHTYPES_FETCH_FIXTURES")
        .map(PathBuf::from)
        .unwrap_or_else(|| {
            PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../spec/fixtures/fetch")
        });
    let dir = dir.canonicalize().unwrap_or(dir);
    if !dir.join("expected.json").is_file() {
        announce(&format!(
            "\nSKIP: no fetch fixtures at {} (spec/fixtures/fetch, generated by the core \
